Shader "SH/plane/grass"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                float3 worldPos = mul(unity_ObjectToWorld, i.origrin_vert1).xyz;
                UNITY_SETUP_INSTANCE_ID(i); //??????
                //float atten = SHADOW_ATTENUATION(i);
                ...
                float grassDiffuise = dot(float3(0,1,0), normalize(worldLightDir)) * 0.5 + 0.5;
                grassDiffuise *= _LightColor0;
                ...
                return float4((final_color*atten).xyz *2,1);
            }
            ENDCG
        }
