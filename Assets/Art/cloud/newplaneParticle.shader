Shader "Unlit/NewUnlitShader"
{
    Properties
    {
        ...
        _UniformAlpha("uniform alpha",range(0,1)) = 1
        _UniformIntensity("uniform intensity",range(0,10)) =1

        _FresnelPower("fresnel power",range(0,10)) = 1
        _DepthFadeTolerance("depth fade tolerance",range(-0.1,0.1)) = 0
        _DepthFadePower("depth fade power",range(0,10)) = 1
    	
    	_EdgeSmoothness("EdgeSmoothness",Range(0,5)) = 0.1
        ...
        Pass
        {
           // Tags { "LightMode" = "ForwardBase" "RenderType"="Transparent""Queue"="Transparent" "IgnoreProjector"="True"}
            ...
            struct v2f
            {
                ...
                float viewSpaceDepth : TEXCOORD5;
                SHADOW_COORDS(3)
            ...
            float _DiffusePower,_UniformAlpha,_FresnelPower,_UniformIntensity,_DepthFadeTolerance,_DepthFadePower,_EdgeSmoothness;
            sampler3D _NoiseTex;
            ...
            {   
                ...
                float alphaNoise = tex3D(_NoiseTex,i.worldPos.xyz * _NoiseTiling + _Time.x);
                alphaNoise = saturate(alphaNoise + _UniformAlpha);

                float fresnelTerm = saturate(dot(i.normal,-viewDir)) * 0.9;
                fresnelTerm = pow(fresnelTerm,_FresnelPower);
                float finalAlpha = saturate(alphaNoise * fresnelTerm ) * _UniformIntensity;
                ...
                trueDepth = Linear01Depth(trueDepth);

                fixed fragDepth = i.viewSpaceDepth/ _ProjectionParams.z * 100;

                float depthMix = smoothstep(0,_DepthFadeTolerance,-(fragDepth - trueDepth));
                depthMix = 1 - pow(depthMix,_DepthFadePower);
                ...
                //return float4(depthMix,depthMix,depthMix,1);
				return float4(col.xyz,finalAlpha * depthMix);
